...
define(function (require) {
    ...
    Game.prototype.update = function () {
        ...
        if (this.blockout) {
            this.blockoutTimer += dt;

            if (this.blockoutTimer > (BLOCKOUT_TIME / 3) && this.nextActiveScene) {
                this.gui.hideView(this.activeScene);
                this.activeScene = this.nextActiveScene;
                this.nextActiveScene = null;
                this.sceneChanged = false;
            }

            if (this.blockoutTimer > (BLOCKOUT_TIME / 2) && !this.sceneChanged) {
                // Enter current scene
                this.gui.setActiveView(this.activeScene);
                this.broadcast("enter_scene_" + this.activeScene);
                this.scenes[this.activeScene].enter();
                this.sceneChanged = true;
            }

            if (this.blockoutTimer > BLOCKOUT_TIME) {
                this.blockout = false;
            }
        }
    ...
    Game.prototype.render = function () {
        var alpha, canvasRect = renderAssistant.getCanvasRect();

        this.scenes[this.activeScene].render(renderAssistant);
        this.scenes[this.activeScene].afterRender(renderAssistant);

        if (this.blockout) {
            if (this.blockoutTimer < BLOCKOUT_TIME / 2) {
                alpha = this.blockoutTimer.toFixed(2);
            } else {
                alpha = BLOCKOUT_TIME - this.blockoutTimer.toFixed(2);
            }
            renderAssistant.drawQuad(0, 0, canvasRect.width, canvasRect.height, "rgba(0, 0, 0, " + alpha + ")");
        }
